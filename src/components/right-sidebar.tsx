...
export default function RightSidebar({ recordMap }) {
    ...
    return (
      <div className='w-64 bg-white overflow-y-auto fixed right-20 top-30 p-5 border-gray-200 border-b'>
        <span className='text-black text-sm font-medium'>On this page</span>
        <ul className='list-none p-0 mt-3'>
          {headings.map((heading, index) => (
            <li key={index} className='mb-1'>
              <a href={`#${heading.id}`} className={` hover:text-gray-500 text-sm font-medium ${selectedId === heading.id ? 'text-sky-600' : 'text-gray-400'}`}>
                {heading.text}
              </a>
            </li>
          ))}
